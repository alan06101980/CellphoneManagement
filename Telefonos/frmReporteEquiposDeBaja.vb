...
Public Class frmReporteEquiposDeBaja
...
#End Region
    ...
    Private Sub frmReporteEquiposDeBaja_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub btnConsultar_Click(sender As Object, e As EventArgs) Handles btnConsultar.Click
        ...
        Try
            ...
            Dim consulta As New SqlCeDataAdapter("SELECT TELEFONO.IDTELEFONO AS NÚMERO, PROVEEDOR AS PROVEEDOR, USUARIO.NOMBREUSUARIO AS USUARIO, MARCA AS MARCA, MODELO AS MODELO, DISPOSITIVO AS DISPOSITIVO, FECHA_HISTORIAL AS FECHA_HISTORIAL, CATEGORIA AS CATEGORIA FROM TELEFONO INNER JOIN HISTORIAL ON TELEFONO.IDTELEFONO = HISTORIAL.IDTELEFONO INNER JOIN PROVEEDOR ON TELEFONO.IDPROVEEDOR = PROVEEDOR.IDPROVEEDOR INNER JOIN USUARIO ON TELEFONO.IDUSUARIO = USUARIO.IDUSUARIO INNER JOIN CATEGORIAS ON HISTORIAL.IDCATEGORIA = CATEGORIAS.IDCATEGORIA INNER JOIN MARCA ON MARCA.IDMARCA = TELEFONO.IDMARCA INNER JOIN MODELO ON MODELO.IDMODELO = TELEFONO.IDMODELO INNER JOIN DISPOSITIVO ON DISPOSITIVO.IDDISPOSITIVO = TELEFONO.IDDISPOSITIVO	WHERE HISTORIAL.IDCATEGORIA = 8  AND  FECHA_HISTORIAL >= @FECHAINICIAL AND FECHA_HISTORIAL <= @FECHAFINAL ORDER BY FECHA_HISTORIAL", cn)
